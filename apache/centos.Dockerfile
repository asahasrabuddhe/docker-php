...
FROM ajitemsahasrabuddhe/centos-base:${base_version}

ARG env=dev
ARG php_version

# Install PHP / Apache / Composer
RUN yum -y install https://dl.fedoraproject.org/pub/epel/epel-release-latest-${{base_version}}.noarch.rpm \
    && yum -y install http://rpms.remirepo.net/enterprise/remi-release-${{base_version}}.rpm \
...
RUN if [ "$env" = "dev" ] ; then apt-get install -y php-xdebug; fi
...
RUN { \
		...
		echo '</Directory>'; \
	} | tee "/etc/httpd/conf.d/vhost.conf"
